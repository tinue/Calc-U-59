...
- **Keyboard** The Mac app now takes input from a physical keyboard. All 45 keys of the face have an unshifted binding (digits and operators where you'd expect them, `Enter` for `=`, `Esc` for CLR, `Backspace` for CE, `Space` for R/S, `a`–`e` for A–E); second functions work as on the real machine, by pressing `'` for 2nd and then the key, with `Shift`+letter as a shorthand for the same thing — so `Shift`+`A` gives A′. The full map is in `reference/AppArchitecture.md` § "Physical Keyboard Mapping". Keys only reach the calculator while it is the focused panel, shown by a thin orange ring; the printer and debug panels are unchanged. iOS and iPadOS are unaffected.
