...
function CalcKey({ kc, scale, onPress, onDown, onUp, forcePressed = false }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap: 2*scale }}>
      {/* Upright, same family as the keycap below \u2014 real hardware's 2nd-function
          legends (log, sin, P\u2192R, x=t, \u2026) are plain, not italicized; confirmed
          against App/Assets.xcassets/ti59_base.imageset/ti59_base.png. */}
      <span style={{
        fontFamily: "var(--font-keycap)",
        fontWeight: 500,
        fontSize: 11 * scale,
      ...
      <button
        onMouseDown={() => { setPressed(true); onDown && onDown(kc.label); }}
        onMouseUp={() => { setPressed(false); onUp && onUp(kc.label); }}
        onMouseLeave={() => { setPressed(false); onUp && onUp(kc.label); }}
        onTouchStart={() => { setPressed(true); onDown && onDown(kc.label); }}
        ...
        onTouchEnd={(e) => { e.preventDefault(); setPressed(false); onUp && onUp(kc.label); }}
        onClick={(e) => {
          if (onDown) {
            ...
            if (e.detail === 0) { onDown(kc.label); onUp && onUp(kc.label); }
          } else if (onPress) {
            onPress(kc.label);
          }
        }}
