...
function ledDrawSegmentsClassic(ctx, rect, segments, segmentOpacity) {
  ...
  const segs = [
    [true, rx + sw, ry, rw - 2 * sw, 0],           // A top
    [false, rMaxX - sw, ry, hh, 1],                // B upper-right
    [false, rMaxX - sw, rMidY, hh, 2],              // C lower-right
    [true, rx + sw, rMaxY - sw, rw - 2 * sw, 3],    // D bottom
    [false, rx, rMidY, hh, 4],                       // E lower-left
    [false, rx, ry, hh, 5],                          // F upper-left
...
function ledDrawDisplay(ctx, cssWidth, cssHeight, snap) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    if (!snap) {
      if (i === 0) { segments = LED_DIGIT_SEGMENTS[0]; hasDot = true; }
    ...
    ledDrawSegmentsClassic(ctx, { x: 0, y: 0, width: digitWidth, height }, segments, segmentOpacity);
    ctx.restore();

    if (hasDot) {
      ledDrawDecimalPoint(ctx, { x, y: 0, width: digitWidth, height });
    }
  }
}

const { useRef: useLedRef, useEffect: useLedEffect } = React;
...
// equivalent for.
function LedDisplayCanvas({ scale, snap, width = 292, height = 36 }) {
...
Object.assign(window, { LedDisplayCanvas });
