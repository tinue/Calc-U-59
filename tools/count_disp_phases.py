...
def analyze_phases(path, show_raw=True, show_visible=True):
    ...
    # Count phases (consecutive same-state entries)
    phases = []
    if states:
    ...
    if show_raw:
        ...
        print("=" * 80)
        phase_str = ' - '.join(f"{state}({count})" for state, count, _ in phases)
        print(f"{phase_str}\n")
    ...
    if show_visible:
        ...
        filtered = []
        for state, time_ms in phases_ms:
        ...
        merged = []
        for state, time_ms in filtered:
            if merged and merged[-1][0] == state:
                merged[-1][1] += time_ms
            else:
                merged.append([state, time_ms])

        phase_str = ' - '.join(f"{state}({time_ms:.0f}ms)" for state, time_ms in merged)
        print(f"{phase_str}\n")

        on_phases = [t for s, t in merged if s == 'ON']
        off_phases = [t for s, t in merged if s == 'OFF']

        print(f"ON phases ({len(on_phases)}):", file=sys.stderr)
        for i, t in enumerate(on_phases, 1):
            print(f"  {i}: {t:.0f}ms ({t/1000:.2f}s)", file=sys.stderr)

        if off_phases:
            print(f"\nOFF phases ({len(off_phases)}):", file=sys.stderr)
            for i, t in enumerate(off_phases, 1):
                print(f"  {i}: {t:.0f}ms ({t/1000:.2f}s)", file=sys.stderr)
